...
- Skill 按原始完整 `skill_name` 归组，保留插件前缀，不拆分版本；同名且无法区分来源的记录会合并。`unlinked` 表示该分组全部记录均未关联中心。
...
- 下载和收藏只在分组能明确解析到唯一中心 Skill ID 时读取，不按名字匹配、不求和调用记录上的重复关联。
...
依赖 V49 账本和原有采集转发链路，无新增迁移、汇总表或定时任务。后端与前端共同发布；先确认 V49 已执行及真实调用入库，再验证管理员查询。不会自动回灌历史日志。
