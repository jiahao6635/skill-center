...
import type { Namespace, NamespaceMember, ManagedNamespace, CreateNamespaceRequest, NamespaceCandidateUser, NamespaceRole, BatchMemberResponse, PagedResponse } from '@/api/types.ts'
import { namespaceApi } from '@/api/client.ts'
import { replaceNamespaceMemberRole } from '@/shared/lib/namespace-member-cache.ts'
import { shouldEnableNamespaceMemberCandidates } from './skill-query-helpers.ts'
...
export function useMyNamespaces({ enabled = true }: { enabled?: boolean } = {}) {
  return useQuery({
